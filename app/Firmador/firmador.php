...
use App\Firmador\XPath;
use App\Firmador\XMLSecEnc;
use App\Firmador\XMLSecurityDSig;
use App\Firmador\XMLSecurityKey;
...
class Firmador
{
    ...
    public function signerXml($pfx, $pin, $input, $output, $path = null)
    {
        ...
        $objSec->addReference($objSec->getXadesNode(), $objSec::SHA1, null, ['force_uri' => false, 'overwrite' => false, "type" => "http://uri.etsi.org/01903#SignedProperties"], [['qualifiedName' => 'xmlns:xades', 'value' => $objSec::XADES]]);

        // Referencia de nodo de información clave
        $objSec->addReference($objSec->getKeyInfoNode(), $objSec::SHA1, null, ['id_ref' => $objSec->reference1Id, 'force_uri' => false, 'overwrite' => false]);

        // Referencia del documento
        $objSec->addReference($xml, $objSec::SHA1, ['http://www.w3.org/2000/09/xmldsig#enveloped-signature'], ['id_ref' => $objSec->reference0Id, 'force_uri' => true]);
